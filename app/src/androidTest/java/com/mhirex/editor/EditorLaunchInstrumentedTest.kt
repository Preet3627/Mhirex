 ...
 * reliable check that the custom-view FQCNs rewritten during the `com.tharunbirla.librecuts` →
 * `com.mhirex.editor` rename still resolve. Those are resolved by reflection at inflate time, so a
...
class EditorLaunchInstrumentedTest {
     ...
     * `src/androidTest/assets/mhirex_test_fixture.mp4` (1s, 64x64, H.264 + silent AAC) rather than
    ...
    private fun testClip(): File {
        ...
        testContext.assets.open("mhirex_test_fixture.mp4").use { input ->

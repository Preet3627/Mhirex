...
import com.mhirex.editor.commands.*
...
fun VideoEditingViewModel.deleteSequenceSegment(index: Int) {
    executeCommand(com.mhirex.editor.commands.MutateProjectCommand("Delete Segment") { project ->
...
fun VideoEditingViewModel.setScrubProxyUri(dependencyId: String, proxyUri: Uri) {
    // Kept as a compatibility entry point for older callers. Proxy publication is metadata, not a
    // user edit, so it must not enter undo history or make the project appear unsaved.
    applyGeneratedProxy(dependencyId, proxyUri)
}

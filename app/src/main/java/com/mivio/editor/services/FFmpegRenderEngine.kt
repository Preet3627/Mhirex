...
class FFmpegRenderEngine private constructor(private val context: Context) {
    ...
    init {
        ...
        try {
            FFmpegKitConfig.enableLogCallback { log ->
                ...
                if (sId != 0L && activeSessions.containsKey(sId)) {
        ...
        withContext(Dispatchers.IO) {
            ...
            activeSessions.keys.toList().forEach { sessionId ->
                runCatching { FFmpegKit.cancel(sessionId) }
                    .onFailure { Log.w(TAG, "Could not cancel FFmpeg session $sessionId: ${it.message}") }
            ...
            val completedSession = suspendCancellableCoroutine<FFmpegSession> { cont ->
                val asyncSession = try {
                    FFmpegKit.executeAsync(
                        command,
                        { completeSession ->
                            // Cancellation can win the race while FFmpeg is winding down.
                            if (cont.isActive) cont.resume(completeSession)
                        },
                        { log ->
                ...
                cont.invokeOnCancellation {
                    ...
                    runCatching { FFmpegKit.cancel(asyncSession.sessionId) }
                        .onFailure { Log.w(TAG, "Could not cancel FFmpeg session ${asyncSession.sessionId}: ${it.message}") }
    ...
    private fun recordSessionLog(log: com.antonkarpenko.ffmpegkit.Log) {
        ...
        val buffer = sessionLogBuffers.getOrPut(sessionId) { ConcurrentLinkedQueue() }
        if (buffer.size < 2000) buffer.add(log)
    }

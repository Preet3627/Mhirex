...
* [ ] **0.17 Phase 0 verification**
  ...
  * **Blockers:**
  ...
  4. **Google Sign-In: sign-in FAILS in the published release; sign-out is untested.** Added after AD-2 as `com.mhirex.editor.auth.*` (Credential Manager, not the deprecated `play-services-auth` client). *Corrected twice on 2026-09-25. First correction: this blocker previously said the flow had "NEVER been exercised on a device", which was true when written and became false. Second correction, same day: it then claimed the earlier `UNREGISTERED_ON_API_CONSOLE` failure was "an environment fault, not a code defect" (no screen lock on the emulator) and treated sign-in as proven. That attribution is withdrawn — sign-in is now **reported failing by a user on a real device**, so one successful emulator run does not validate the shipped build.* **What is actually established:** compiles, clean `assembleDebug`, `testDebugUnitTest` 4/4, `INTERNET` present in the APK, `default_web_client_id` and `google_app_id` correctly compiled into the release APK, and the release APK is signed with the project release key (SHA-1 `43:CD:DA:44:34:CF:34:77:91:6B:DE:4B:C6:7F:2A:A8:14:7F:5C:C5`, SHA-256 `093197df…46bb6f`), the same key as `v1.0-beta8`. Installing that exact published APK on the emulator and pulling it back off the device confirms the installed build carries the release certificate, so **the emulator and the GitHub release match, and a version or certificate mismatch between them is ruled out.** On an API 36 emulator with a real Google account a sign-in did once complete — chooser returned, profile persisted to `librecuts_prefs.xml` (`profile_signed_in`, display name, email, photo URL), avatar downloaded to `cache/user_avatar.jpg` (1909 bytes), no ID token stored — but the build used for that run is not established, so it cannot be cited as validation of the release. **Leading cause of the release failure: the release certificate's SHA-1 is not registered on the OAuth client.** `app/google-services.json` can neither confirm nor deny this — it contains no `certificate_hash` key at all and lists only a Web OAuth client (`client_type: 3`), with no Android client (`client_type: 1`), so it is not a genuine Firebase export and proves nothing about console state. Registering the SHA-1s is a console action that cannot be performed or verified from this repo. **Still unverified: sign-out**, so `clearCredentialState()` and the avatar deletion on sign-out are untested code paths — and sign-out is the path that removes a personal photo from disk, so treat it as unproven until someone taps it. **Not yet done:** reproduce on a real device with `adb logcat -c && adb logcat -s Auth.Api.Credentials` while tapping **Sign in with Google**, to separate an unregistered-certificate rejection from any other cause. Release blocker: users cannot attach a profile until the SHA-1s are registered.

...
* [ ] **0.17 Phase 0 verification**
  ...
  * **Blockers:**
  ...
  4. **Google Sign-In: sign-in is now proven on a device; sign-out is not.** Added after AD-2 as `com.mhirex.editor.auth.*` (Credential Manager, not the deprecated `play-services-auth` client). *Corrected 2026-09-25 — this blocker previously said the flow had "NEVER been exercised on a device", which was true when written and is now false.* **Verified:** compiles, clean `assembleDebug`, `testDebugUnitTest` 4/4, `INTERNET` present in the APK, OAuth web client ID matches `google-services.json`; and on an API 36 emulator with a real Google account, a full sign-in completed — Google's chooser returned, the profile persisted to `librecuts_prefs.xml` (`profile_signed_in`, display name, email, photo URL) and the avatar downloaded to `cache/user_avatar.jpg` (1909 bytes). No ID token is stored. **Still unverified: sign-out**, so `clearCredentialState()` and the avatar deletion on sign-out are untested code paths — and sign-out is the path that removes a personal photo from disk, so treat it as unproven until someone taps it. Note the earlier `UNREGISTERED_ON_API_CONSOLE` failure was an *environment* fault, not a code defect: the emulator had no screen lock registered, so Play Services account reauth could not complete. Debug builds still need the debug keystore SHA-1 added to the OAuth client before sign-in will work on any other machine.

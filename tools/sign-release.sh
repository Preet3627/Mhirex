...
# That record now lives in docs/signing-fingerprints.md, and this script ends by checking the
# certificate it just produced against it, so a keystore that was regenerated by accident is caught
# here instead of by a user who cannot install the update.
#
# Usage
# -----
...
echo
echo "=== signing certificate vs docs/signing-fingerprints.md ==="
# This used to only print the digest next to the advice that it "must match earlier releases", with
# nothing on disk to match it against, so the check could not actually be made. The record now
# exists and is enforced here: a release signed by an unregistered certificate is refused rather
# than staged, because it cannot install over an existing Mhirex and cannot sign in.
FIRST="$OUT/$(ls "$OUT" | head -1)"
if "$ROOT/tools/check-signing-fingerprint.sh" --apk "$FIRST"; then
  echo "  This is the same key every published release was signed with."
else
  status=1
  echo
  echo "Signing certificate does not match the permanent record. Do not upload." >&2
fi
...
if [ $status -eq 0 ]; then
  ...
  echo "One or more checks FAILED. Do not upload." >&2
fi
exit $status
